...
public class PlayListHelper {
    ...
    public static ArrayList<Audio> getAllSongsOfPlaylist(Context context, Long pid) {
        ...
        try {
            ...
            Cursor cursor = resolver.query(uri, PROJECTION, null, null, MediaStore.Audio.Playlists.Members.DEFAULT_SORT_ORDER);
            if (cursor != null) {
                while (cursor.moveToNext()) {
    ...
    public static void renamePlaylist(ContentResolver resolver, long id, String newName) {
        ...
        // The requested name is already called; nothing to do
        if (existingId == id)
            return;
        // There's already a playlist with this name. Delete it.
        if (existingId != -1)
